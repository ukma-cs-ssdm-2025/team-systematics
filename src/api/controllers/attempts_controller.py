...
class AttemptsController:
    ...
    def _register_routes(self) -> None:
        self.router.add_api_route(
            "/{attempt_id}/answers", self.add_answer, methods=["POST"],
            response_model=Answer, status_code=status.HTTP_201_CREATED,
            summary="Save or update an answer within an attempt",
        )
        self.router.add_api_route(
            "/{attempt_id}/submit", self.submit, methods=["POST"],
            response_model=Attempt,
            summary="Submit attempt",
        )

    async def add_answer(self, attempt_id: UUID, payload: AnswerUpsert) -> Answer:
        return self.service.add_answer(attempt_id, payload)

    async def submit(self, attempt_id: UUID) -> Attempt:
